...
static void inference_engine_finalizer(JSRuntime *rt, JSValue val) {
    ...
    engine->shutdown(); // join all worker threads before closing the handle
...
// new InferenceEngine({ models: [{ model_class, path }] })
static JSValue inference_engine_ctor(JSContext *ctx, JSValueConst new_target,
    ...
    if (JS_IsArray(ctx, models)) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (cls && path) engine->add_worker(cls, path);
...
// do_inference(worker_id, { session_id, prompt }, (err, token) => {})
static JSValue js_do_inference(JSContext *ctx, JSValueConst this_val,
                                int argc, JSValueConst *argv) {
    if (argc < 3)
        return JS_ThrowTypeError(ctx, "do_inference(worker_id, packet, callback)");
    ...
    int worker_id;
    JS_ToInt32(ctx, &worker_id, argv[0]);

    return engine->js_do_inference(ctx, worker_id, argv[1], argv[2]);
}
