...
class Server {
    ...
    struct ClientState {
        ...
        static void on_alloc_buffer_cb(uv_handle_t* handle, size_t suggested_size, uv_buf_t *buf);
...
void Server::on_alloc_buffer_cb(uv_handle_t* handle, size_t suggested_size, uv_buf_t *buf) {
...
void Server::on_peer_connected(uv_stream_t* server_stream, int status) {
    ...
    if(uv_accept(server_stream, (uv_stream_t*) &client->socket) == 0) {  // start listening to incoming requests
        ...
        int r = uv_read_start((uv_stream_t*)&client->socket, on_alloc_buffer_cb, on_read_cb);
    }
}
...
Server::Server(int portNum, const char* portAddr): portNum(portNum), portAddr(portAddr) {}
